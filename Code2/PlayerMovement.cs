...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
       // Jumping
        ...
        {
            if (!GravityWarper.isGravityReversed)  // Gets the bool from the GravityWarper class to see if gravity is warped so that jumping works both directions.
            {
                rb.AddForce(Vector3.up * jumpForce, ForceMode.Impulse);
            }
            else
            {
                // If gravity is reversed, apply an opposite force to counteract gravity for jumping
    ...
    void OnCollisionEnter(Collision collision)
    {
     ...
     IEnumerator ActivateSpeedBoost(){ // speed boost to make the player move twice as fast while walking and sprinting.
        isSpeedBoosted = true;
